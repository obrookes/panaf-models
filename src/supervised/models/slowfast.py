...
class SlowFastEmbedder(nn.Module):
    def __init__(self, model_name="slowfast_r50"):
        ...
        self.backbone = nn.Sequential(*list(slowfast.children())[0][:-1])
        self.res_head = create_res_basic_head(
            in_features=2304, out_features=652, pool=None
        )
        self.fc1 = nn.Linear(in_features=652, out_features=128)
        self.fc2 = nn.Linear(in_features=128, out_features=9)

    def forward(self, x):
        emb = self.fc1(self.res_head(self.backbone(x)))
        pred = self.fc2(emb)
        return emb, pred
